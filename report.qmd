...
```{r p-values, echo = F, warning = F, message = F}
## Math
math_full <- lm(MathScore ~ ., data = data_cleaned %>% select(-c(ReadingScore, WritingScore)))
...
```{r BIC, warning = F, message = F, echo = F}
# Math
...
```{r adj R2}
mat = data_cleaned %>% select(-c(WritingScore, ReadingScore))

# Printing the 2 best models of each size, using the adjusted R^2 criterion:
leaps(x = mat %>% select(1:11), y = mat[,11], nbest = 2, method = "adjr2")

# Function regsubsets() performs a subset selection by identifying the "best" model that c
# a certain number of predictors. By default "best" is chosen using SSE/RSS (smaller is better)
b = regsubsets(MathScore ~ ., data = state_ln)
rs = summary(b)
```
